...
- sending API keys:
    - as part of the url: `https://myserver.com/api/customers?apiKey=thisIsTheApiKey` (bad practise, exposed, browsers
      save
      the history)
...
- Provide two or more authentication factors
- 2FA - two-factor authentication
- 1st factor - username and password
- other factors:
    - things we know - what was your first car?
    - things we have - phone (OTP to SMS), email account (verification code sent to email)
    - things we are - fingerprint, face
    - the code/OTP/token has a short validity period

## OAuth2

- protects API, yet allows access to many consumers
- delegated authorization problem
- access without giving a password
- OAuth2:
    - limited access to protected resource
    - user does not give a password
...
Example:
You bok and pay for the hotel. You arrive there. You don't want to give your credit card, so you go to the front desk,
telling your reservation number for validation. The hotel staff checks their registration list and confirm that you are
have the silver package. You fill out some paperwork and then the hotel staff gives you an access card, based on your
package.
You use that card to access the resources of the hotel.

- grant = the hotel reservation number (it gives the app permission to retrieve an access token on behalf of the user).
  OAuth2 define 4 grant types
- hotel access card = access token - a long string credential used to access the resources (usually has a limited time
  and scope)
- scope = ability to read/write certain data


- OAuth2 overview:

1. Client (application) sends an authorization request to the resource owner (user)
2. resource owner responds with an authorization grant
3. the client sends an authorization grant to an authorization server (it is used to validate the authz)
4. authorization server sends back an access token
5. client sends an access token to resource server
6. it sends back the protected resource
...
- most secure; e.g. when you go to some website and login with Google, it requires specific access, i.e. your permission
  to read your Google Drive or access your profile
- Example: app X needs access to your Google Drive to read data from it

1. App X
2. Your Google (resource owner) login page appears
3. Google consent form shows up - consent to read the Google Drive data

- With this authorization, you as the resource owner authorized app X to access Google Drive data

4. Google authorization server issues an authorization code to app X
5. using this authz code, app X sends an access token request to Google authorization server

- Why is this secure?
    - app X never sees the username/password needed for the resource server (Google in this case)
- 3-legged flow
- PKCE when you cannot store the secret token
- suitable for 3rd party app

### Implicit

- the authz returns an access token directly without an authorization code
- intended to be used by user-agent clients (SPA)

### Password

- username + password for access token
- the application collects the user's password
- app & backend owner are the same entity
    - username and password are sent to backend API
    - it uses the client ID + secret and sends it to OAuth2 server
    - it returns an access token to backend API
    - backend API returns it to the client
    - username/password are only known to the backend API
- alternatively, application sends the username/password and oauth credential
    - OAuth server calls the backend API to validate username and password
    - if valid, OAuth server returns the access token to client
    - it then uses it to access the resource of the server (validated by OAuth server)

### Client credentials

- client exchanges the credentials (client ID + secret) with OAuth server
- OAuth server issues the access token to a client which then uses it to access the resources

#### When to use what?

1. Client credentials
   Suitable for: typically, the app is also the resource owner
   Trustworthiness: highly trusted apps, written by internal developer or with a trusted business relationship with the
   API
   provider. Apps that need to access resource on their own behalf.
   Sample use case: B2B (extranet), intranet

2. Password:
   Suitable for: application that has its own username and password (user management)
   Trustworthiness: trusted apps written by internal or trusted 3rd party developers
   Sample use case: intranet sites, portals, mobile app that created & maintained by internal dev

#### Refresh

- OAuth access token is limited in time
- gain a new access token when expired
- refresh sometimes not considered as flow

### Authorization server

- roles:
    - register credentials (client ID & secret)
    - give/revoke credentials
    - provides access tokens
    - access token scope

- Authorization server has to know client ID & secret
- Resource server needs just OAUTH access token
- these two should be separated (with different databases and evolution strategy)
- does not need to build them ourselves
- many providers - free or licensed
- API management provides the authz server
- Examples:
  - okta.com
  - auth0.com
  - ory.sh
